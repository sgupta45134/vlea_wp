...
if ( ! function_exists( '_wp_render_title_tag' ) ) :
	function th_theme_slug_render_title() {
		?>
		<title><?php wp_title('|', true, 'right'); ?></title>
	<?php
	}
	add_action( 'wp_head', 'th_theme_slug_render_title' );
endif;
    ...
    if ($th_screen->id === 'toplevel_page_masterslider') {
        ...
        if ( get_user_meta( $user_id, 'th_ms_install_dismissed', true ) !== '1' ) { ?>
            <div class="notice notice-info">
                <p><?php _e('Looking for the sliders from the live demo? <a href="http://themovation.helpscoutdocs.com/article/198-master-slider-activation" target="_blank">Check out this article for how to import them</a>.', 'entrepreneur'); ?> <a class="th-dismiss" href="?page=masterslider&th-ms-install-dismissed">Dismiss</a></p>
            </div>
            <?php
...
function themo_child_theme_setup() {
    load_child_theme_textdomain( 'entrepreneur', get_stylesheet_directory() . '/languages' );
}
...
function themo_custom_excerpt_more( $output ) {
	if ( (has_excerpt() || themo_has_more()) && ! is_attachment() && get_post_type() != 'themo_tour' && get_post_type() != 'themo_portfolio') {
		$output .= ' &hellip; <a href="' . esc_url(get_permalink()) . '">' . esc_html__('Read More', 'entrepreneur') . '</a>';
	}
	return $output;
}
...
function themo_ot_meta_box_post_format_quote($array,$pages) {
	...
	$array['fields'] = array(
		array(
			...
			'label'   => '',
			'desc'    => esc_html__( 'Quote', 'entrepreneur' ),
			'std'     => '',
			'type'        => 'text',
			'rows'        => '4',
		),
...
function themo_ot_meta_box_post_format_video($array,$pages) {
	...
	$array['fields'] = array(
		array(
			...
			'desc'    => sprintf( wp_kses_post( __( '(ex. http://vimeo.com/link-to-video). You can find a list of supported oEmbed sites in the %1$s.', 'entrepreneur' )), '<a href="http://codex.wordpress.org/Embeds" target="_blank">' . esc_html__( 'Wordpress Codex', 'entrepreneur' ) .'</a>' ),
			'std'     => '',
			'type'    => 'text'
		),
		array(
			...
			'desc'    => wp_kses_post(__( 'Use the built-in <code>[video]</code> shortcode here.', 'entrepreneur' )),
			'std'     => '',
			'type'    => 'textarea'
		)
	);
...
function th_register_required_plugins() {
	...
	$plugins = array(
        ...
        array(
            ...
            'source'    => 'https://out.themovation.com/th-widget-pack',
            'required'  => true,
        ),
        array(
    ...
    // Booked
    if ( get_theme_mod('themo_tgmpa_booked',true) ){
        array_push($plugins, array(
            'name'      => 'Booked', // The plugin name.
            'slug'      => 'booked', // The plugin slug (typically the folder name).
            'source'    => 'https://out.themovation.com/booked', //get_template_directory() . '/plugins/th-widget-pack.zip', // The plugin source.
            'required' => false,
        ));
    }
	 ...
	 */
	$config = array(
		'id'           => 'entrepreneur',                 // Unique ID for hashing notices for multiple instances of TGMPA.

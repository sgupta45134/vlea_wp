    ...
    class Envato_Theme_Setup_Wizard {
        ...
        public function admin_menus() {
            ...
            if ( $this->is_submenu_page() ) {
                ...
                $add_subpage_function( $this->parent_slug, esc_html__( 'Theme Setup', 'entrepreneur' ), esc_html__( 'Theme Setup', 'entrepreneur' ), 'manage_options', $this->page_slug, array(
                    $this,
                    'setup_wizard',
                ) );
            } else {
                add_theme_page( esc_html__( 'Theme Setup', 'entrepreneur' ), esc_html__( 'Theme Setup', 'entrepreneur' ), 'manage_options', $this->page_slug, array(
                    $this,
                    'setup_wizard',
                ) );
            }
        ...
        public function init_wizard_steps() {
            ...
            $this->steps = array(
                'introduction' => array(
                    'name'    => esc_html__( 'Introduction', 'entrepreneur' ),
            ...
            if( $this->should_we_install_child_theme() ) {
                $this->steps['child_theme'] = array(
                    'name' => esc_html__('Customization Support', 'entrepreneur'),
            ...
            if ( class_exists( 'TGM_Plugin_Activation' ) && isset( $GLOBALS['tgmpa'] ) ) {
                $this->steps['default_plugins'] = array(
                    'name'    => esc_html__( 'Plugins', 'entrepreneur' ),
                    'view'    => array( $this, 'envato_setup_default_plugins' ),
                    'handler' => '',
                );
            }
            $this->steps['updates']         = array(
                'name'    => esc_html__( 'Updates', 'entrepreneur' ),
...
//            }
        ...
        public function create_child_theme() {
            ...
            if ( wp_mkdir_p( $child_dir ) ) {
                ...
                wp_die( esc_html__( 'Error: theme folder not writable. Please install Child Theme manually or just go back and skip this step.', 'entrepreneur' ) );
            }
        }
...
";
        ...
        public function setup_wizard() {
            ...
            wp_localize_script( 'envato-setup', 'envato_setup_params', array(
                ...
                'verify_text'      => esc_html__( '...verifying', 'entrepreneur' ),
            ) );
    ...
    public function setup_wizard_header() {
        ...
        <head>
            ...
            echo '<t';
            echo 'itle>' . esc_html__( 'Theme &rsaquo; Setup Guide', 'entrepreneur' ) . '</ti' . 'tle>'; ?>
        ...
        <?php if ( 'next_steps' === $this->step ) : ?>
            <a class="wc-return-to-dashboard"
               href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'entrepreneur'); ?></a>
        <?php endif; ?>
        </body>
        <?php
        ...
        public function envato_setup_introduction() {
            ...
            if ( false && isset( $_REQUEST['debug'] ) ) {
                ...
                ?>
                <h1><?php printf( esc_html__( 'Upgrade?', 'entrepreneur' ), wp_get_theme() ); ?></h1>
                <p><?php esc_html_e( 'It looks like you may have recently upgraded to this theme. Great! This setup guide will help ensure all the default settings are correct. It will also show some information about your new website and support options.', 'entrepreneur' ); ?></p>
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'entrepreneur' ); ?></a>
                    <a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
                       class="button button-large"><?php esc_html_e( 'Not right now', 'entrepreneur' ); ?></a>
                </p>
                <?php
            } else if ( get_option( 'envato_setup_complete', false ) ) {
                ?>
                <h1><?php printf( esc_html__( 'Are you sure? It looks like you have already run the setup guide.', 'entrepreneur' ), wp_get_theme() ); ?></h1>
                <p><?php _e( 'If you would like to switch demo presets and start from a clean install, please use the <a href="https://wordpress.org/plugins/wordpress-reset/" target="_blank">WordPress Reset plugin</a> before rerunning the Setup Guide. This will remove all content and reset your WordPress install to its factory settings. ', 'entrepreneur' ); ?></p>
                <p><?php esc_html_e( 'If you wish to rerun the Setup Guide, please make sure to select the same preset demo you selected in the original run.', 'entrepreneur' ); ?></p>
                <p><?php esc_html_e( 'Are you sure you want to run it again?', 'entrepreneur' ); ?></p>
                <ul class="no-style">
                    <li>
                        <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                           class=" button button-next button-large"><?php esc_html_e( 'Run Setup Guide Again', 'entrepreneur' ); ?></a>
                    </li>
                    <li>
                        <form method="post">
                            <input type="hidden" name="reset-font-defaults" value="yes">
                            <!--input type="submit" class="button-primary button button-large button-next"
                                   value="<?php esc_attr_e( 'Reset font style and colors', 'entrepreneur' ); ?>" name="save_step"/-->
                ...
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
                       class="button-primary button button-large"><?php esc_html_e( 'Cancel', 'entrepreneur' ); ?></a>
                </p>
                <?php
            } elseif(isset($_REQUEST['page']) && $_REQUEST['page'] ==  'entrepreneurchildtheme-setup') {
                ?>
                <h1><?php printf( esc_html__( 'Nice work! Ready to continue?', 'entrepreneur' ), wp_get_theme() ); ?></h1>
                <p><?php printf( esc_html__( 'Let\'s start installing a few plugins that help make this theme run smoothly.', 'entrepreneur' ), wp_get_theme() ); ?></p>
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'entrepreneur' ); ?></a>
                    <a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
                ...
                ?>
                <h1><?php printf( esc_html__( 'Thank you for choosing Entrepreneur! Ready to get setup?', 'entrepreneur' ), wp_get_theme() ); ?></h1>
                <p><?php printf( esc_html__( 'To make your life easier, we have provided you with a 3 minute step by step installer. It is the best place to get started with the theme setup. Click "Let\'s Go" to continue. ', 'entrepreneur' ), wp_get_theme() ); ?></p>
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'entrepreneur' ); ?></a>
                    <a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
        ...
        public function envato_setup_introduction_save() {
            ...
            if ( ! empty( $_POST['reset-font-defaults'] ) && $_POST['reset-font-defaults'] == 'yes' ) {
                ...
                ?>
                <p>
                    <strong><?php esc_html_e( 'Options have been reset. Please go to Appearance > Customize in the WordPress backend.', 'entrepreneur' ); ?></strong>
                </p>
                <?php
                return true;
            }
        ...
        public function envato_setup_default_plugins() {
            ...
            ?>
            <h1><?php esc_html_e( 'Theme Plugins', 'entrepreneur' ); ?></h1>
            <form method="post">
                ...
                if ( count( $plugins['all'] ) ) {
                    ?>
                    <p><?php esc_html_e( 'Your website needs a few essential plugins. Don\'t worry, you can add and remove plugins later on from within WordPress. The following plugins need to be installed or updated:', 'entrepreneur' ); ?></p>
                    ...
                    echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.', 'entrepreneur' ) . '</strong></p>';
                } ?>
                ...
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       ...
                       data-callback="install_plugins"><?php esc_html_e( 'Continue' , 'entrepreneur'); ?></a>
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button button-large button-next"><?php esc_html_e( 'Skip this step' , 'entrepreneur'); ?></a>
                    <?php wp_nonce_field( 'envato-setup' ); ?>
                </p>
            </form>
            <?php
        ...
        public function ajax_plugins() {
            if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
                wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'entrepreneur' ) ) );
            }
            $json = array();
            ...
            foreach ( $plugins['activate'] as $slug => $plugin ) {
                if ( $_POST['slug'] == $slug ) {
                    $json = array(
                        ...
                        'action2'       => - 1,
                        'message'       => esc_html__( 'Activating Plugin', 'entrepreneur' ),
                    );
                    break;
                }
            }
            foreach ( $plugins['update'] as $slug => $plugin ) {
                if ( $_POST['slug'] == $slug ) {
                    $json = array(
                        ...
                        'action2'       => - 1,
                        'message'       => esc_html__( 'Updating Plugin', 'entrepreneur' ),
                    );
                    break;
                }
            }
            foreach ( $plugins['install'] as $slug => $plugin ) {
                if ( $_POST['slug'] == $slug ) {
                    $json = array(
                        ...
                        'action2'       => - 1,
                        'message'       => esc_html__( 'Installing Plugin', 'entrepreneur' ),
                    );
                    break;
                }
            }
            ...
            if ( $json ) {
                ...
                wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success','entrepreneur' ) ) );
            }
            exit;
        ...
        private function _content_default_get() {
            ...
            foreach ( $available_content as $post_type => $post_data ) {
                if ( count( $post_data ) ) {
                    ...
                    $content[ $post_type ] = array(
                        ...
                        'description'      => sprintf( esc_html__( 'Create default %s as seen on the demo.', 'entrepreneur' ), $post_type_title ),
                        'pending'          => esc_html__( 'Pending.', 'entrepreneur' ),
                        'installing'       => esc_html__( 'Installing.', 'entrepreneur' ),
                        'success'          => esc_html__( 'Success.', 'entrepreneur' ),
            ...
            $content['widgets'] = array(
                'title'            => esc_html__( 'Widgets', 'entrepreneur' ),
                'description'      => esc_html__( 'Insert default sidebar widgets as seen on the demo.', 'entrepreneur' ),
                'pending'          => esc_html__( 'Pending.', 'entrepreneur' ),
                'installing'       => esc_html__( 'Installing Default Widgets.', 'entrepreneur' ),
                'success'          => esc_html__( 'Success.', 'entrepreneur' ),
            ...
            $content['settings'] = array(
                'title'            => esc_html__( 'Settings', 'entrepreneur' ),
                'description'      => esc_html__( 'Configure default settings.', 'entrepreneur' ),
                'pending'          => esc_html__( 'Pending.', 'entrepreneur' ),
                'installing'       => esc_html__( 'Installing Default Settings.', 'entrepreneur' ),
                'success'          => esc_html__( 'Success.', 'entrepreneur' ),
            ...
            $content['forms'] = array(
                'title'            => esc_html__( 'Forms', 'entrepreneur' ),
                'description'      => esc_html__( 'Add Default Forms.', 'entrepreneur' ),
                'pending'          => esc_html__( 'Pending.', 'entrepreneur' ),
                'installing'       => esc_html__( 'Adding Default Forms.', 'entrepreneur' ),
                'success'          => esc_html__( 'Success.', 'entrepreneur' ),
        ...
        public function envato_setup_default_content() {
            ?>
            <h1><?php esc_html_e( 'Demo Content', 'entrepreneur' ); ?></h1>
            <form method="post">
                <?php if ( $this->is_possible_upgrade() ) { ?>
                    <p><?php esc_html_e( 'It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'entrepreneur' ); ?></p>
                <?php } else { ?>
                    <p><?php printf( esc_html__( 'Getting started is easier with demo content. We recommend checking all boxes as you can always choose to remove content from within WordPress after setup.','entrepreneur' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
                ...
                <table class="envato-setup-pages" cellspacing="0">
                    ...
                    <tr>
                        <td class="check"></td>
                        <th class="item"><?php esc_html_e( 'Item', 'entrepreneur' ); ?></th>
                        <th class="description"><?php esc_html_e( 'Description', 'entrepreneur' ); ?></th>
                        <th class="status"><?php esc_html_e( 'Status', 'entrepreneur' ); ?></th>
                    </tr>
                    </thead>
                    <tbody>
                ...
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button-primary button button-large button-next"
                       data-callback="install_content"><?php esc_html_e( 'Continue', 'entrepreneur' ); ?></a>
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'entrepreneur' ); ?></a>
                    <?php wp_nonce_field( 'envato-setup' ); ?>
                </p>
        ...
        public function ajax_content() {
            ...
            if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
                wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No content Found', 'entrepreneur' ) ) );
            }
            ...
            if ( $json ) {
                ...
                wp_send_json( array(
                    'error'   => 1,
                    'message' => esc_html__( 'Error', 'entrepreneur' ),
                    'logs'    => $this->logs,
                    'errors'  => $this->errors,
                ) );
            }
                               ...
                               //if($post_id == 29 || $post_id == 20){
                                ...
                                if ('_elementor_edit_mode' == $meta_key && 'builder' === $meta_val){
                                    ...
                                    if(!isset($elm_page_settings_array) || empty($elm_page_settings_array)) {
                                        ...
                                        $group1 = array("home-main","home-masonry-blog","home-shop",
                                            "home-coming-soon","about-us","careers","support-help-faq",
                                            "gallery","portfolio","portfolio-4-column","portfolio-5-column",
                                            "blog-masonry","blog-masonry-full","blog-masonry-sidebar","blog-standard-2",
                                            "blog-standard-sidebar","shop-2","shop-sidebar","cart","checkout","pricing");
                                        $group2 = array("contact","contact-2","home-video");
                                        //$group3 = array("pricing");

                                        $th_post = get_post($post_id);
                                        $th_slug = $th_post->post_name;
                                        $th_post_type = get_post_type( $post_id);


                                        if (in_array($th_slug, $group1) || $th_post_type == 'themo_portfolio') {
                                            $elm_page_settings_array = array ('hide_title'=>'yes','themo_transparent_header'=>'on','themo_alt_logo'=>'on');
                                            ...
                                            $elm_page_settings_array = array ('hide_title'=>'yes','themo_transparent_header'=>'on','themo_header_content_style'=>'dark');
                                            update_post_meta( $post_id, "_elementor_page_settings", $elm_page_settings_array );
        ...
        private function _fetch_remote_file( $url, $post ) {
            ...
            if ( ! $upload || $upload['error'] ) {
                ...
                if ( is_array( $response ) && ! empty( $response['body'] ) && $response['response']['code'] == '200' ) {
                    ...
                    return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond', 'entrepreneur' ) );
                }
                ...
                if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
                    ...
                    return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size', 'entrepreneur' ) );
                }

                if ( 0 == $filesize ) {
                    @unlink( $upload['file'] );

                    return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded', 'entrepreneur' ) );
                }

                if ( ! empty( $max_size ) && $filesize > $max_size ) {
                    @unlink( $upload['file'] );

                    return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote file is too large, limit is %s', 'entrepreneur' ), size_format( $max_size ) ) );
                }
            }
        ...
        public function _content_install_settings() {
            ...
            // Sometimes our import site contain the dtbwp_site_style theme mode, this gets imported and overwrites
            // the local value seleted. This is a temp fix, just hold it and set it after the option import.
            $themo_hold_style = get_theme_mod('dtbwp_site_style',$this->get_default_theme_style());
            ...
            // Reset style to the local value selected. Overwrite any imported value.
            set_theme_mod( 'dtbwp_site_style', $themo_hold_style );
            ...
            switch (get_theme_mod('dtbwp_site_style',$this->get_default_theme_style())){
                ...
                case 'style1':
                    // BARBER
                    $homepage = get_page_by_title( 'Barber Home' );
                    break;

                case 'style2':
                    // COACH
                    $homepage = get_page_by_title( 'Coach Home' );
                    break;

                case 'style3':
                    // CONSULTANT
                    $homepage = get_page_by_title( 'Consultant Home' );
                    break;

                case 'style4':
                    // CONTRACTOR
                    $homepage = get_page_by_title( 'Contractor Home' );
                    break;

                case 'style5':
                    // HEALTH CARE
                    $homepage = get_page_by_title( 'Healthcare Home' );
                    break;

                case 'style6':
                    // STYLIST
                    $homepage = get_page_by_title( 'Stylist Home' );
                    break;

                case 'style7':
                    // TRAINER
                    $homepage = get_page_by_title( 'Trainer Home' );
                    break;

                case 'style8':
                    // YOGA
                    $homepage = get_page_by_title( 'Yoga Home' );
                    break;

                case 'style9':
                    // REMOTE LEARNING
                    $homepage = get_page_by_title( 'Remote Learning Home' );
                    break;

                case 'style10':
                    // SMALL BIZ
                    $homepage = get_page_by_title( 'Home Main' );
                    break;
            ...
            //error_log("HOMEPAGE ID ". $homepage->ID);

            //error_log("STYLE " . get_theme_mod('dtbwp_site_style',$this->get_default_theme_style()));

            if ( $homepage->ID ){
                //error_log("update_option page_on_front ". $homepage->ID);
        ...
        public function envato_setup_child() {
            ?>
            <form method="post">
                <h1><?php esc_html_e( 'Customization Support', 'entrepreneur' ); ?></h1>
                ...
                <p class="envato-setup-actions step">
                    <input type="submit" class="button-primary button button-large button-next"
                           value="<?php esc_attr_e( 'Continue', 'entrepreneur' ); ?>" name="save_step"/>
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                       class="button button-large button-next"><?php esc_html_e( 'Skip this', 'entrepreneur' ); ?></a>
                    <?php wp_nonce_field( 'envato-setup' ); ?>
                </p>
            </form>
            <?php
        ...
        public function envato_setup_color_style() {
            ...
            ?>
            <h1><?php esc_html_e( 'Theme Demo Style', 'entrepreneur' ); ?></h1>
            <form method="post">
                <p><?php esc_html_e( 'Which demo would you like to install? You can always customize your page layout and style settings from within WordPress after setup. Choose your favorite and click continue. ', 'entrepreneur' ); ?></p>
...
//            wp_redirect( esc_url_raw( $this->get_next_step_link() ) );
        ...
        public function envato_setup_logo_design() {
            ...
            ?>
            <h1><?php esc_html_e( 'Logo', 'entrepreneur' ); ?></h1>
            <form method="post">
                <p><?php esc_html__( 'Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard.','entrepreneur' ); ?></p>
                ...
                <table>
                    <tr>
                        ...
                        <td>
                            <a href="#" class="button button-upload"><?php esc_html_e( 'Upload New Logo', 'entrepreneur' ); ?></a>
                        </td>
                    </tr>
                </table>
        ...
        public function envato_setup_updates() {
            ?>
            <h1><?php esc_html_e( 'Theme Updates', 'entrepreneur' ); ?></h1>
            <?php if ( function_exists( 'envato_market' ) ) { ?>
                <form method="post">
                    ...
                    if ( count( $my_items ) ) {
                        ...
                        <p class="envato-setup-actions step">
                            <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                               class="button button-large button-next button-primary"><?php esc_html_e( 'Continue', 'entrepreneur'); ?></a>
                        </p>
                        <?php
                    } else {
                        ?>
                        <p><?php esc_html_e( 'Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.', 'entrepreneur' ); ?></p>
                        <p>When an update becomes available it will show in the Dashboard with an option to install.</p>
                        <p>
                        ...
                        <p class="envato-setup-actions step">
                            <input type="submit" class="button-primary button button-large button-next"
                                   value="<?php esc_attr_e( 'Login with Envato', 'entrepreneur' ); ?>" name="save_step"/>
                            <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                               class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'entrepreneur' ); ?></a>
                            <?php wp_nonce_field( 'envato-setup' ); ?>
                        </p>
        ...
        public function envato_setup_customize() {
            ...
            <p class="envato-setup-actions step">
                <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                   class="button button-primary button-large button-next"><?php esc_html_e( 'Continue', 'entrepreneur' ); ?></a>
            </p>
        ...
        public function envato_setup_help_support() {
            ...
            <p class="envato-setup-actions step">
                <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                   class="button button-primary button-large button-next"><?php esc_html_e( 'Agree and Continue', 'entrepreneur' ); ?></a>
                <?php wp_nonce_field( 'envato-setup' ); ?>
            </p>
            <?php
        }
        ...
        public function envato_setup_ready() {
            ...
            <a href="https://twitter.com/share" class="twitter-share-button"
               ...
               data-text="<?php echo esc_attr( 'I just setup my new website using the Entrepreneur #WordPress theme from #ThemeForest' ); ?>"
            ...
            <h1><?php esc_html_e( 'Your website is ready!', 'entrepreneur' ); ?></h1>
            ...
            <div class="envato-setup-next-steps">
                <div class="envato-setup-next-steps-first">
                    <h2><?php esc_html_e( 'Next steps', 'entrepreneur' ); ?></h2>
                    <ul>
                        <li class="setup-product"><a class="button button-primary button-large"
                                                     href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'View your new website!', 'entrepreneur' ); ?></a>
                        </li>
                        <li class="setup-product"><a class="button button-next button-large"
                                                     href="https://twitter.com/themovation"
                                                     target="_blank"><?php esc_html_e( 'Follow @themovation on Twitter', 'entrepreneur' ); ?></a>
                        </li>
                    </ul>
                </div>
                <div class="envato-setup-next-steps-last">
                    <h2><?php esc_html_e( 'Resources', 'entrepreneur' ); ?></h2>
                    <ul>
                        <li class="documentation"><a href="http://themovation.helpscoutdocs.com/"
                                                     target="_blank"><?php esc_html_e( 'Read the Theme Documentation', 'entrepreneur' ); ?></a>
                        </li>
                        <li class="support"><a href="https://themovation.ticksy.com/"
                                               target="_blank"><?php esc_html_e( 'Get Help and Support', 'entrepreneur' ); ?></a></li>
                    </ul>
                </div>
            </div>
            <?php
        ...
        public function envato_market_admin_init() {
            ...
            if ( ! empty( $_POST['oauth_session'] ) && ! empty( $_POST['bounce_nonce'] ) && wp_verify_nonce( $_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username ) ) {
                ...
                if ( is_wp_error( $response ) ) {
                    ...
                    $class         = 'error';
                    echo "<div class='".esc_attr($class)."'><p>" . sprintf( esc_html__( 'Something went wrong while trying to retrieve oauth token: %s', 'entrepreneur' ), $error_message ) . '</p></div>';
                } else {
            ...
            add_settings_section(
                ...
                sprintf( esc_html__( 'Login for %s updates', 'entrepreneur' ), $this->envato_username ),
            ...
            add_settings_field(
                ...
                esc_html__( 'oAuth Login', 'entrepreneur' ),
                ...
                if ( !$option || empty($option['oauth']) || empty( $option['oauth'][ $this->envato_username ] ) || empty($envato_items) || empty($envato_items['items']) || !envato_market()->items()->themes( 'purchased' )) {
                    ...
                    if ( ! $dissmissed_time || $dissmissed_time < strtotime('-7 days') ) {
                        ...
                        <div class="notice notice-warning notice-dtbwp-themeupdates is-dismissible">
                            <p><?php
                                _e( 'Please activate ThemeForest updates to ensure you have the latest version of this theme.','entrepreneur' );
                                ?></p>
                            <p>
                                <?php printf( __( '<a class="button button-primary" href="%s">Activate Updates</a>','entrepreneur' ),  esc_url($this->get_oauth_login_url( admin_url( 'admin.php?page=' . envato_market()->get_slug() . '' ) ) ) ); ?>
                            </p>
                        </div>

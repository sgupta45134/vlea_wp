...
class REST_API{
    ...
    public function ultp_route_post_data($prams,$local=false) {
        ...
        if(isset($prams['taxonomy'])){ // taxonomy  slug__val__slug__val
            $relation = isset($prams['queryRelation']) ? $prams['queryRelation'] : 'OR';
            $tax_arr = array('relation' => $relation);
        ...
        if(isset($prams['queryAuthor'])){ 
            $args['author'] = esc_attr($prams['queryAuthor']);
        }

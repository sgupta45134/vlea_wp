...
class User_Register extends Module_Base {
	...
	public function get_style_depends() {
        if ($this->ep_is_edit_mode()) {
            ...
            return [ 'element-pack-font', 'ep-user-register' ];
        }
    }
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .bdt-button-modal svg' => 'fill: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .bdt-button-modal:hover svg' => 'fill: {{VALUE}};',
				],
			]
		);
	...
	public function form_fields_render_attributes() {
		...
		if ( isset( $settings['is_needed_password_input'] ) && $settings['is_needed_password_input'] ) {
			$this->add_render_attribute(
				[
					'password_input' => [
						'type'        => 'password',
	...
	public function render() {
		...
		if ( is_user_logged_in() && ! Element_Pack_Loader::elementor()->editor->is_edit_mode() ) {
            ...
            <div class="bdt-alert bdt-alert-warning" bdt-alert>
                <a class="bdt-alert-close"><i class="ep-close" aria-hidden="true"></i></a>
	...
	public function user_register_form() {
        ...
        <form id="bdt-user-register<?php echo esc_attr( $id ); ?>"
			...
			<?php if ( isset( $settings['is_needed_password_input'] ) && $settings['is_needed_password_input'] ): ?>

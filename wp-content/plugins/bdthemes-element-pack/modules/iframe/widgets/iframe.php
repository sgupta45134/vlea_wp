...
class Iframe extends Module_Base {
	...
	protected function _register_controls() {
		...
		$this->add_responsive_control(
			'iframe_size',
			[
				'label'       => esc_html__( 'Iframe Width', 'bdthemes-element-pack' ) . BDTEP_NC,
				'type'        => Controls_Manager::SLIDER,
				'range'       => [
					'px' => [
						'min' => 180,
	...
	public function render() {
		...
		$custom_ratio = ($settings['slider_size_ratio']['width'] && $settings['slider_size_ratio']['height']) ? $settings['slider_size_ratio']['width'] . "/" . $settings['slider_size_ratio']['height'] : '600/1280';
		...
		if($settings['show_iframe_device']) {
			$this->add_render_attribute( 'iframe', 'style', 'aspect-ratio:'. $ratio);
		} else {

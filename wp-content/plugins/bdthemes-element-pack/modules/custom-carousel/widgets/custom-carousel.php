...
class Custom_Carousel extends Module_Base {
	...
	protected function _register_controls() {
		...
		$this->register_navigation_style_controls( 'custom-carousel');
		...
		$this->add_control(
			...
			[
				...
				'type'      => Controls_Manager::SLIDER,
				'selectors' => [
					'{{WRAPPER}} .bdt-custom-carousel-item .bdt-overlay i' => 'font-size: {{SIZE}}{{UNIT}};',
				],
				'condition' => [
					'overlay' => 'icon',
				],
		...
		$this->add_responsive_control(
			...
			[
				...
				'type'    => Controls_Manager::SLIDER,
				'selectors' => [
					'{{WRAPPER}} .bdt-custom-carousel-item .bdt-overlay' => 'margin: {{SIZE}}px; max-width: calc(100% - ({{SIZE}}px * 2));',
				],
			]
		);
	...
	protected function render_slide_image( array $slide, $element_key, array $settings ) {
		...
		<?php if ( $settings['overlay'] ) : ?>
			<div <?php echo $this->get_render_attribute_string( 'overlay-settings' ); ?>>
				...
				<?php else : ?>
					<i class="ep-<?php echo esc_attr($settings['icon']); ?>" aria-hidden="true"></i>
				<?php endif; ?>
			</div>
		<?php endif; ?>

...
class Tags_Cloud extends Module_Base
{
    ...
    public function get_script_depends() {
        return ['tags-cloud', 'tags-exCanvas'];
    }
    ...
    public function wp_generate_tag_cloud($tags, $animaitedSetting, $args = '')
    {
        ...
        if ($animaitedSetting == 'animaitedTags') {
            ...
            foreach ($tags_data as $key => $tag_data) {
                ...
                $a[]   = sprintf(
                    '<li><a href="%1$s"%2$s class="%3$s" data-weight="%4$s"%5$s>%6$s%7$s</a></li>', esc_url($tag_data['url']), $tag_data['role'], esc_attr($class), esc_attr(str_replace(',', '.', $tag_data['font_size'])), $tag_data['aria_label'], esc_html($tag_data['name']), $tag_data['show_count']
            ...
            foreach ($tags_data as $key => $tag_data) {
                ...
                $a[]   = sprintf(
                    ...
                    '<span data-weight="%4$s"><a  href="%1$s"%2$s class="%3$s" %5$s>%6$s%7$s</a> </span>', esc_url($tag_data['url']), $tag_data['role'], esc_attr($class), esc_attr(str_replace(',', '.', $tag_data['font_size'])), $tag_data['aria_label'], esc_html($tag_data['name']), $tag_data['show_count']
